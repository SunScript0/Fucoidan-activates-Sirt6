...
setwd("/scratch/fmorandi/internal/Ali/atac_seq")
...
dir.create("./results_combined", showWarnings = F)
...
# Tidy sample names
all(colnames(counts_ocr) == colnames(counts_re))
colnames(counts_ocr) = str_extract(colnames(counts_ocr), "/([^/]+)_clean.bam", group=1)
colnames(counts_re) = colnames(counts_ocr)

# Read meta
meta = read.table("./meta.txt", header = T)

# Read featureCounts report
fcreport = read.table(paste0(paths$data, "/07_peaksets_and_tables/counts_combined.tsv.summary"), header=T, row.names=1)
...
save(counts_ocr, counts_re, norm_ocr, norm_re, pinfo, reinfo, meta,
     file=paste0(paths$results, "/prepro_combined.Rdata"))
